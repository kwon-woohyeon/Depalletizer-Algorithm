...
namespace Project
{
    public partial class Form8 : Form
    {
        ...
        {
            DataTable data = db.LoadCompanyData(); // LoadData 호출
        ...
        {
            string insertQuery = "INSERT INTO project.test ( name, address, date, total) VALUES ( @name, @address, @date, @total)";
            ...
            try
            {
                ...
                if (!lastRow.IsNewRow) // 마지막 행이 새 행인지 확인
                {
                    ...
                    string newname = lastRow.Cells["name"].Value?.ToString() ?? string.Empty;
                    string newaddress = lastRow.Cells["address"].Value?.ToString() ?? string.Empty;
                    string newdate = lastRow.Cells["date"].Value?.ToString() ?? string.Empty;
                    string newtotal = lastRow.Cells["total"].Value?.ToString() ?? string.Empty;

                    // 필수 데이터가 비어 있는 경우 처리
                    if ( string.IsNullOrWhiteSpace(newname) || string.IsNullOrWhiteSpace(newdate) || string.IsNullOrWhiteSpace(newaddress) || string.IsNullOrWhiteSpace(newtotal))
                    {
                    ...
                    using (MySqlConnection conn = new MySqlConnection(connectionString))
                    {
                        ...
                        using (MySqlCommand insertCmd = new MySqlCommand(insertQuery, conn))
                        {
                            ...
                            insertCmd.Parameters.AddWithValue("@address", newaddress);
                            insertCmd.Parameters.AddWithValue("@date", newdate);
                            insertCmd.Parameters.AddWithValue("@total", newtotal);
        ...
        {
            string query = "UPDATE project.test SET name = @name, address = @address, date = @date, total = @total WHERE id = @id";
        ...
        {
            string query = "DELETE FROM project.test WHERE id = @id";
            ...
            try
            {
                ...
                if (dataGridView1.SelectedRows.Count > 0) // 선택된 행이 있는지 확인
                {
                    ...
                    // 삭제할 데이터 가져오기
                    string id = selectedRow.Cells["id"].Value.ToString(); // 삭제 기준 값
